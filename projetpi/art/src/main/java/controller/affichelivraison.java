...
public class affichelivraison implements Initializable {
    ...
    private List<livraison> getDatatrie() {
        ...
        try {
            for (livraison item : sp.selectAlltri()) {
                ...
                liv.setNom_coomande(item.getNom_coomande());
                liv.setNom_livreur(item.getNom_livreur());
                livres.add(liv);
            }
    ...
    private List<livraison> getData2() {

        servicePerson sp = new servicePerson();
        List<livraison> livres = new ArrayList<>();
        livraison liv;
        try {
            for (livraison item : sp.selectByname(rechercher.getText())) {
                System.out.println("gguu");

                liv = new livraison();
                liv.setID(item.getID());
                System.out.println(item.getAdresse());
