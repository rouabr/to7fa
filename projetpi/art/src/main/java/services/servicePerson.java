...
public class servicePerson implements CRUD<livraison>{
    ...
    public void insertOne(livraison person) throws SQLException {
        String req = "INSERT INTO livraison(date_livraison, adresse_livraison, status_livraison, frais_livraison, id_livreur, id_commande) VALUES (?, ?, ?, ?, ?, ?)";
        try (PreparedStatement st = cnx.prepareStatement(req)) {
            ...
            st.setInt(5, person.getId_livreur());
            st.setInt(6, person.getId_commande());
    ...
    public void updateOne(livraison liv) throws SQLException {
        String req = "UPDATE livraison SET date_livraison = ?, adresse_livraison = ?, status_livraison = ?, frais_livraison = ?, id_livreur = ?, id_commande = ?  WHERE id_livraison = ?";
        try (PreparedStatement pst = cnx.prepareStatement(req)) {
            pst.setString(1, liv.getDate());
            pst.setString(2, liv.getAdresse());
            pst.setString(3, liv.getStatus());
            pst.setFloat(4, liv.getFrais());
            pst.setInt(5, liv.getId_livreur());
            pst.setInt(6, liv.getId_commande());
