...
class PanierController extends AbstractController
{
    ...
    #[Route('/count', name: 'app_count', methods: ['GET', 'POST'])]

    public function add_count(CommandeRepository $commandesRepository,EntityManagerInterface $entityManager): Response
    {
        // Get the count of sales since last month
        $salesCount = $commandesRepository->countSalesthismonth();
        $salesLast = $commandesRepository->countSales();
    

        $historicalData = $commandesRepository->getHistoricalDataForLastMonth();

        $probabilityOfEachOutcome = 1 / count($historicalData);

        // Calculate the probability of winning the parlay
        $parlayProbability = pow($probabilityOfEachOutcome, count($historicalData));

        

        // Format the probability as a percentage with two decimal places
        $parlayProbabilityPercentage = number_format($parlayProbability * 100, 2);
  
        // Inside your controller or service
$salesData = $this->getDoctrine()->getRepository(Commande::class)->findAll();


        $salesByMonth = [];
        foreach ($salesData as $sale) {
            $month = $sale->getDateCommande()->format('Y-m');
            if (!isset($salesByMonth[$month])) {
                $salesByMonth[$month] = 0;
            }
            $salesByMonth[$month] += $sale->getPrixCommande();
        }
        $months = array_keys($salesByMonth);
        ...
        return $this->render('dashboard/index.html.twig', [
            ...
            'salesLast' => $salesLast,
            'parlayProbabilityPercentage' => $parlayProbabilityPercentage,
            'months' => $months,
