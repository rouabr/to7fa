...
FileChooser fileChooser = new FileChooser();
    ...
    private void modifierMusee(ActionEvent event) {
        try {
            ...
            if (museeSelectionne != null) {
                ...
                if (tfnom.getText().isEmpty() || tfadresse.getText().isEmpty() || tfville.getValue().isEmpty() || tftickets.getText().isEmpty() || tfdescription.getText().isEmpty() || (selectedImagePath == null || selectedImagePath.isEmpty())) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Erreur de saisie");
                    alert.setContentText("Veuillez remplir tous les champs");
                    alert.show();
    ...
    void getData(MouseEvent event) {
        ...
        if (musee != null) {
            ...
            selectedImagePath = musee.getImage_path();
    ...
    public void showMusees1(){
        ...
        coldescription.setCellValueFactory(new PropertyValueFactory<Musee,String>("description"));
        coltickets.setCellValueFactory(new PropertyValueFactory<Musee,Integer>("nbr_tickets_disponibles"));
        FilteredList<Musee> filteredData = new FilteredList<>(list, b-> true);
        searchField.textProperty().addListener((observable , oldValue, newValue) -> {
            filteredData.setPredicate(Musee -> {
                if(newValue.isEmpty() || newValue.isBlank() || newValue == null){
                    return true;
                }
                String searchKeyword = newValue.toLowerCase();
                if(Musee.getNom_musee().toLowerCase().indexOf(searchKeyword) > -1){
                    return true;
                }
                else if(Musee.getDescription().toLowerCase().indexOf(searchKeyword) > -1){
                    return true;
                }
                else if(Musee.getAdresse().toLowerCase().indexOf(searchKeyword) > -1){
                    return true;
                }
                else if(Musee.getVille().toLowerCase().indexOf(searchKeyword) > -1){
                    return true;
                }else
                    return false;
            });
        ...
        sortedData.comparatorProperty().bind(tvmusee.comparatorProperty());
        tvmusee.setItems(sortedData);
    }
