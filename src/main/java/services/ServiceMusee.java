...
public class ServiceMusee implements CRUD<Musee> {
    ...
    public String getCity(int museeId) {
        String city = "";
        System.out.println(city);
        String query = "SELECT ville FROM Musee WHERE id_musee = ?";
        try {
            if (cnx == null || cnx.isClosed()) {
                // reconnect to the database
                cnx = DriverManager.getConnection("jdbc:mysql://localhost:3306/pidev", "root", "");
            ...
            e.printStackTrace();
            System.err.println("Error while getting the city: " + e.getMessage());
        }
        return city;
    }
    ...
    @Override
    public List<Musee> selectByname(String ch) throws SQLException {
        List<Musee> oeuv = new ArrayList<>();
        String req = "SELECT * FROM musee WHERE nom_musee LIKE ?";
        PreparedStatement statement = cnx.prepareStatement(req);
        statement.setString(1,  ch + "%");
        ResultSet rs = statement.executeQuery(); // Use the prepared statement here
        while (rs.next()) {
            Musee p = new Musee();
            p.setId_musee(rs.getInt("id_musee"));
            System.out.println("id ==== " + p.getId_musee());
            p.setNom_musee(rs.getString("nom_musee"));
            p.setAdresse(rs.getString("adresse"));
            p.setNbr_tickets_disponibles(rs.getInt("nbr_tickets_disponibles"));
            p.setDescription(rs.getString("description"));
            p.setImage_path(rs.getString("image_musee"));
            oeuv.add(p);
        }
        return oeuv;
    }
